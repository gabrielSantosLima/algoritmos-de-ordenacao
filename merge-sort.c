...
typedef struct ARRAY
{
	...
	int *data;	// Primeiro elemento do vetor
...
ARRAY *new_array(int size)
{
	...
	narray->length = 0;
	narray->size = size;
	narray->data = (int *)malloc(sizeof(int) * size); // Aloca um vetor de int de tamanho sizeof(int) * size
	return narray;
}
...
void merge(ARRAY *array, int inicio, int meio, int fim)
{
	...
	for (int i = 0; i < n1; i++)
	{
		aux1->data[i] = array->data[inicio + i];
	}
	for (int j = 0; j < n2; j++)
	{
		aux2->data[j] = array->data[meio + 1 + j];
	}
